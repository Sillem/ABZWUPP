...
class GUI(object):
    ...
    def create_formularz(self):
        ...
        selected_form, url, levels = self.get_form(bs, selected_language, languages, forms)
        response = requests.request("GET", url, headers = headers, data = payload)
        bs = BeautifulSoup(response.content, "html.parser")
        ### Wybór stopnia studiów (licencjat,inżynier itp.) ###
        selected_level, url = self.get_level(selected_language, languages, selected_form, forms, levels)
        ### Wybór wydziału ###
